...
class AddSurveyorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        storageReference = FirebaseStorage.getInstance().reference.child("surveyor_images")
        ...
        selectedImageUri?.let {
            ...
            fileReference.putFile(it).addOnSuccessListener { taskSnapshot ->
                fileReference.downloadUrl.addOnSuccessListener { uri ->
                    val newItem = Surveyor(name, specialty, introduction, location, proposals, price, uri.toString())
                    saveSurveyorToDatabase(newItem)
                }
            }.addOnFailureListener { e ->
                Toast.makeText(this@AddSurveyorActivity, "Failed to upload image: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }

    private fun saveSurveyorToDatabase(newItem: Surveyor) {
        val database = FirebaseDatabase.getInstance().reference.child("surveyors")
        val surveyorId = database.push().key
        surveyorId?.let {
            database.child(it).setValue(newItem).addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    Toast.makeText(this@AddSurveyorActivity, "Failed to add expert: ${task.exception?.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
